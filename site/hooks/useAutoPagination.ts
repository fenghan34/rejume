...
interface Page { html: string, pageNumber: number, pageStyle: CSSProperties }
...
export function useAutoPagination(html: string, options: AutoPaginationOptions) {
  const containerRef = useRef<HTMLDivElement>(null)
  const [pages, setPages] = useState<Page[]>()

  const { pageSize: { width, height }, padding: { x: paddingX, y: paddingY } } = options
