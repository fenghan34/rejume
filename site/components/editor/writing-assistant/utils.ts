...
export function pushEdit(editor: MonacoEditor, type: 'insert' | 'replace', text: string) {
  ...
  const selections = model.pushEditOperations([selection], [
    {
      text,
      range: {
        startLineNumber,
    ...
    (inverseEditOperations) => {
      const { startLineNumber, startColumn, endLineNumber, endColumn } = inverseEditOperations[0].range
      const selection = new Selection(startLineNumber, startColumn, endLineNumber, endColumn)
      return [selection]
    }
  )
