...
export function AuthForm({ type }: { type: 'login' | 'signup' }) {
  ...
  const getCallbackURL = () => {
    return new URL(redirectURL || '/dashboard', window.location.origin).href
  }
  ...
  }, [router, session?.user?.id, redirectURL])

  return (
    <div className="h-full flex justify-center">
      <div className="w-md p-6 mt-[10vh] space-y-10">
        <div className="text-center">
          ...
          >
            <Link
              href={`/${isSignup ? 'login' : 'signup'}${redirectURL ? `?redirect=${redirectURL}` : ''}`}
            >
              {isSignup
